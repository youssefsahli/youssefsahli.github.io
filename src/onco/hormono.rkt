#lang scribble/manual
@title{traitement hormonologique du cancer du sein}
@section{Castration physique @emph{définitive}}
@para{Elle se fait par chirurgie et radiotherapie localisée.}
@section{Castration chimique, @emph{temporaire et réversible}}
@para{Blocage de l'axe hypothalamo-hypophyso-gonadique pour empêcher la sécrétion hormonale}
@itemlist[
 @item{Analogues GnRH}
 @item{Progestatifs
 @itemlist[
 @item{Médroxyprogestérone DEPO-PRODASONE}
 @item{Acétate de Mégestrol MEGACE}]}]